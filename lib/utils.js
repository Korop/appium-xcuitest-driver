...
async function getPIDsUsingPattern (pattern, opts = {}) {
  ...
  try {
    ...
    log.debug(`pgrep pids output for pattern '${pattern}' is '${stdout}', error='${stderr}'`);
    if (typeof (stdout) === 'undefined' || stdout == null || stdout.length === 0) {
      log.debug(`Could not find any process by pattern 'pgrep ${args.join(' ')}'`);
      return null;
    }
    if (multi) {
    ...
    log.debug(`Found process pid '${pid}'`);
    return isNaN(pid) ? null : `${pid}`;
  } catch (err) {
    log.debug(`'pgrep ${args.join(' ')}' didn't detect any matching processes. Return code: ${err.code}, error message: ${err.message}`);
    return null;
  }
}
