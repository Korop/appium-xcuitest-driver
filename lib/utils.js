...
async function getPIDsUsingPattern (pattern, opts = {}) {
  ...
  try {
    //const {stdout, stderr} = await exec('bash', ['-c', `pgrep -nif '${pattern}'`]);
    const {stdout, stderr} = await exec('pgrep', args);
    log.debug(`xcuitest-driver: pgrep pids line for pattern '${pattern}' is '${stdout}', error='${stderr}'`);
    if (typeof (stdout) === 'undefined' || stdout == null || stdout.length === 0) {
      log.debug(`xcuitest-driver: could not find a process by pattern 'pgrep ${args.join(' ')}'`);
      return null;
    }
    if (multi) {
    ...
    //const pid = parseInt(stdout.replace(/\s/g, ''), 10);
    const pid = parseInt(stdout, 10);
    log.debug(`xcuitest-driver:  number pid='${pid}'`);
    if (isNaN(pid)) {
      log.debug(`xcuitest-driver: Cannot parse process id from 'pgrep ${args.join(' ')}' output: '${stdout}', error='${stderr}'`);
      return null;
    }
    log.debug(`xcuitest-driver: return pid='${pid}'`);
    return `${pid}`;
  } catch (err) {
    log.debug(`xcuitest-driver: 'pgrep ${args.join(' ')}' didn't detect any matching processes. Return code: ${err.message}`);
    return null;
  }
}
...
async function killAppUsingPattern (pgrepPattern) {
  for (const signal of [2, 15, 9]) {
    if (!await getPIDsUsingPattern(pgrepPattern)) {
      log.debug(`killAppUsingPattern: do not find process anymore by pattern '${pgrepPattern}'`);
      return;
    }
    const args = [`-${signal}`, '-if', pgrepPattern];
    try {
      log.debug(`killAppUsingPattern: 'pkill ${args.join(' ')}'`);
      await exec('pkill', args);
    } catch (err) {
      log.debug(`killAppUsingPattern: pkill ${args.join(' ')} -> ${err.message}`);
    }
    await B.delay(100);
  }
}
...
async function resetXCTestProcesses (udid, isSimulator, opts = {}) {
  const processPatterns = [`xcodebuild.*appium-xcuitest-driver.*${udid}`];
  if (opts.derivedDataPath) {
    log.debug(`resetXCTestProcesses: opts.derivedDataPath=${opts.derivedDataPath}`);
    processPatterns.push(`xcodebuild.*${opts.derivedDataPath}.*${udid}`);
  }

  if (opts.bootstrapPath) {
    log.debug(`resetXCTestProcesses: opts.bootstrapPath=${opts.bootstrapPath}`);
    processPatterns.push(`xcodebuild.*${opts.bootstrapPath}.*${udid}`);
  }
