...
class XCUITestDriver extends BaseDriver {
  ...
  async deleteSession () {
    ...
    log.debug(`deleteSession: check syslog isEmpty(this.logs)=${_.isEmpty(this.logs)}`);
    if (!_.isEmpty(this.logs)) {
      log.debug('deleteSession: syslog stopCapture');
